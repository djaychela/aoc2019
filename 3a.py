...
for idx, element in enumerate(route_1[1:]):
    if idx %1000 == 0:
        print(f'comparing {idx} - current minimum: {minimum_distance}')
    if element in route_2:
        print(f'Match found - {element[0], element[1]}')
        minimum_distance = min((abs(element[0]) + abs(element[1])), minimum_distance)
        print(f'Minimum distance now {minimum_distance}')
